...
# remove antibody capture layer from RNA assay
Layers(scCHANTS)
scCHANTS@assays$RNA@layers$`counts.Antibody Capture` <- NULL
...
# remove old layer
scCHANTS@assays$RNA@layers$`counts.Gene Expression` <- NULL
...
Cells(scCHANTS_nonegs) %>%  length()
Cells(scCHANTS_nonegs[["RNA"]])  %>%  length()# Cells in RNA assay
Cells(scCHANTS_nonegs[["ADT"]])  %>%  length()# Cells in ADT assay (if present)
Cells(scCHANTS_nonegs[["HTO"]]) %>%  length()
