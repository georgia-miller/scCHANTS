...
VlnPlot(scCHANTS_pbmc, features = "nFeature_RNA", group.by = "timepoint", y.max = 200)
...
# can look in more detail if needed e.g.
VlnPlot(scCHANTS_t, features = "nFeature_RNA", group.by = "timepoint", y.max = 400)
...
```{r}
# can subset straightaway based on QC metrics e.g. :
#scCHANTS_t <- subset(scCHANTS_t, subset = nFeature_RNA > 200  & nFeature_RNA < 2500 & percent_mt < 5) 
...
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$nFeature_RNA < 150 & scCHANTS_t$QC == "initial",
  "low_nFeature",
  scCHANTS_t$QC
)
...
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$nFeature_RNA > 2000 & scCHANTS_t$QC == "initial",
  "high_nFeature",
  scCHANTS_t$QC
)
...
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$percent_mt > 5 & scCHANTS_t$QC == "initial",
  "high_mt",
  scCHANTS_t$QC
)
...
# label combined: low_nFeature + high_mt
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$nFeature_RNA < 150 & scCHANTS_t$percent_mt > 5 & scCHANTS_t$QC != "pass",
  "low_nFeature_high_mt",
  scCHANTS_t$QC
)
...
# label combined: high_nFeature + high_mt
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$nFeature_RNA > 2000 & scCHANTS_t$percent_mt > 5 & scCHANTS_t$QC != "pass",
  "high_nFeature_high_mt",
  scCHANTS_t$QC
)
...
# finally, label cells that pass all filters
scCHANTS_t$QC <- ifelse(
  scCHANTS_t$nFeature_RNA >= 150 &
    scCHANTS_t$nFeature_RNA <= 2000 &
    scCHANTS_t$percent_mt <= 5 &
    scCHANTS_t$QC == "initial",
  "pass",
  scCHANTS_t$QC
)
...
pca5 <- wrap_plots(gene_trees, ncol = 3)
pca5
ggsave("PCA_mean_gene_exp_clusteringtree.png", plot = pca5, device = png, height = 40, width = 45, unit = "cm", path = paste0(file_path, "/T_plots/PCA/clustree") )
...
pca6 <- wrap_plots(gene_trees2, ncol = 3)
pca6
ggsave("PCA_median_gene_exp_clusteringtree.png", plot = pca6, device = png, height = 40, width = 45, unit = "cm", path = paste0(file_path, "/T_plots/PCA/clustree") )
...
pca7 <- wrap_plots(CD4_trees, ncol = 3)
pca7
ggsave("PCA_mean_CD4_gene_exp_clusteringtree.png", plot = pca7, device = png, height = 40, width = 45, unit = "cm", path = paste0(file_path, "/T_plots/PCA/clustree") )
...
pca8 <- wrap_plots(CD8_trees, ncol = 3)
pca8
ggsave("PCA_mean_CD8_gene_exp_clusteringtree.png", plot = pca8, device = png, height = 55, width = 45, unit = "cm", path = paste0(file_path, "/T_plots/PCA/clustree") )

```

## Run umap and visualise
```{r}
scCHANTS_t <- RunUMAP(scCHANTS_t, dims = 1:9)
...
pca14 <- wrap_plots(umap_plots, ncol = 3)
pca14
ggsave("PCA_compare_res_umaps.png", plot = pca14, device = png, height = 25, width =30, unit = "cm", path = paste0(file_path, "/T_plots/PCA/") )


# plot RNA counts, mt and rb contamination
pca15 <- VlnPlot(scCHANTS_t, features = c("nFeature_RNA", "nCount_RNA", "percent_mt", "percent_rb"), ncol = 2, alpha = 0.3, group.by = "RNA_snn_res.0.7")
pca15
ggsave("PCA_compare_clusters_violinplots.png", plot = pca15, device = png, height = 22, width = 25, unit = "cm", path = paste0(file_path, "/T_plots/PCA/") )
...
pca16 <- ggplot(cluster_counts, aes(x = cluster, y = cell_count, fill = cluster)) +
  ...
  theme(legend.position = "none")
pca16
ggsave("PCA_compare_cluster_size_barplots.png", plot = pca16, device = png, height = 10, width = 10, unit = "cm", bg = "white", path = paste0(file_path, "/T_plots/PCA/") )
...
```{r}
# plot umap coloured by cell_type and cell_type_lowerres
pca17 <- DimPlot(scCHANTS_t, reduction = "umap", group.by = "sample_id")
pca17
# ggsave("PCA_sample_id_umap.png", plot = pca17, device = png, height = 10, width =10, unit = "cm", path = paste0(file_path, "/T_plots/PCA/") )

pca18 <- DimPlot(scCHANTS_t, reduction = "umap", group.by = "timepoint")
pca18
# ggsave("PCA_timepoint_umap.png", plot = pca18, device = png, height = 10, width =10, unit = "cm", path = paste0(file_path, "/T_plots/PCA/") )
