...
However, to save the processed seurat object, should run as a batch script, found at ~/R/scCHANTS/script_1.11_save_processed_20250616_pbmc_rds.R or ~/R/scCHANTS/script_1.11_save_processed_20250616_t_rds.R
...
# Script 1.1a Load and HTO demultiplex
...
```{r}
file_path <- "/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/plots"
```
...
# gunzip("/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_benchmark/matrix.mtx.gz", temporary = TRUE, remove = FALSE)
# gunzip("/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_benchmark/barcodes.tsv.gz", temporary = TRUE, remove = FALSE)
# gunzip("/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_benchmark/features.tsv.gz", temporary = TRUE, remove = FALSE)
...
scCHANTS_data <- Read10X(data.dir = "/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_benchmark/", gene.column=2)
...
adt_mat <- GetAssayData(scCHANTS, assay = "ADT", layer = "counts")
has_adt <- colSums(adt_mat) > 0

table(scCHANTS$HTO_classification.global, has_adt)
...
scCHANTS_singlets <- subset(scCHANTS, subset = HTO_classification.global == "Singlet")
...
d7 <- DimPlot(scCHANTS_singlets, group.by = "HTO_classification") +
  labs(title = "RNA UMAP HTO_classification")
d7
# ggsave("RNA_HTO_classification_UMAP.png", plot = d7, device = png, height = 10, width = 15, unit = "cm", path = paste0(file_path, "/demultiplex/") )

# show only a subset
subset1 <- subset(scCHANTS_singlets, subset = HTO_classification %in% c("HTO-1", "HTO-2", "HTO-3") )
d8 <- DimPlot(subset1, group.by = "HTO_classification") +
  labs(title = "RNA UMAP HTO_classification on HTO-1:3")
d8
# ggsave("RNA_HTO_1to3_classification_UMAP.png", plot = d8, device = png, height = 10, width = 15, unit = "cm", path = paste0(file_path, "/demultiplex/") )

# should be same as above
DimPlot(subset1, group.by = "HTO_maxID") +
  labs(title = "RNA UMAP HTO_maxID on HTO-1:3")

subset2 <- subset(scCHANTS_singlets, subset = HTO_classification %in% c("HTO-7", "HTO-8", "HTO-9") )
...
# ggsave("HTO_umis_per_cell_histogram.png", plot = d9, device = png, bg = "white", height = 7, width = 12, unit = "cm", path = paste0(file_path, "/demultiplex/") )
...
metadata_samples <- read.csv("/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/sample_metadata.csv")
...
# Processing (QC, PCA, clustering, UMAPs)


# 1.1a PBMC pre-processing
...
qc3 <- ggplot(pbmc_qc, mapping = aes(x = Var1, y = percent, fill = Var1)) +
  ...
  theme_minimal()
qc3
# ggsave("QC_barplot_postQC.png", plot = qc3, device = png, bg = "white", height = 8, width = 15, unit = "cm", path = paste0(file_path, "/PBMC_plots/QC/") )
```
...
```{r}
# saveRDS(object=scCHANTS_pbmc, file ="/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_scCHANTS_pbmc_processed.Rds")
...
# write.csv(pbmc_markers,"/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_scCHANTS_pbmc_markers.Rds")
...
qc3 <- ggplot(t_qc, mapping = aes(x = Var1, y = percent, fill = Var1)) +
  ...
  theme_minimal()
qc3
# ggsave("QC_barplot_postQC.png", plot = qc3, device = png, bg = "white", height = 8, width = 15, unit = "cm", path = paste0(file_path, "/T_plots/QC/") )
```
...
```{r}
# saveRDS(object=scCHANTS_t, file ="/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_scCHANTS_t_processed.Rds")
...
```{r}
# must join layers to do find markers
! check joining layers is the right thing to do if run again
scCHANTS_pbmc[["ADT"]] <- JoinLayers(scCHANTS_pbmc)
scCHANTS_pbmc[["RNA"]] <- JoinLayers(scCHANTS_pbmc)

# find markers for every cluster compared to all remaining cells, report only the positive ones
t_markers <- FindAllMarkers(scCHANTS_t, only.pos = TRUE)

# write.csv(t_markers,"/scratch/prj/id_hill_sims_wellcda/scCHANTS/20250616_benchmark/20250616_scCHANTS_t_markers.Rds")
